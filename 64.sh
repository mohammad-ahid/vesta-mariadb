...
software="nginx httpd mod_ssl mod_ruid2 mod_extract_forwarded mod_fcgid
    ...
    phpMyAdmin awstats webalizer vsftpd MariaDB-server MariaDB-client exim dovecot clamd
...
rpm -qa > $tmpfile
for pkg in exim bind-9 MariaDB-server httpd nginx vesta; do
...
# Install MariaDB repo v10.0
echo "[mariadb]" > $MARIADB_REPO
echo "name=MariaDB" >> $MARIADB_REPO
echo "baseurl=http://yum.mariadb.org/10.0/centos6-amd64" >> $MARIADB_REPO
echo "enabled=1" >> $MARIADB_REPO
echo "gpgcheck=1" >> $MARIADB_REPO
echo "gpgkey=https://yum.mariadb.org/RPM-GPG-KEY-MariaDB" >> $MARIADB_REPO
...
if [ -z "$disable_remi" ]; then 
    yum -y --disablerepo=* --enablerepo="base,updates,nginx,epel,vesta,remi,mariadb" \
        install $software
else
